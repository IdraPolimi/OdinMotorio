function [ t,a_f ] = metap_learning( s,s_i,c,r,t )
...
%   t = t + n*(r*ones(size(t),1) + g*(t'*basis_metap(s_2,a,b)/sum(basis_metap(s_2,a,b))) - t).*(basis_m./sum(basis_m));
%   Update con g=0
basis_m = basis_metap(s,a,b);
t = t + n*(r*ones(size(t),1) - t).*(basis_m./sum(basis_m));
a_f = (1/(1+c))*(gradiente_v(s)/(sum(basis_m)^2)+c*s_i);
end
